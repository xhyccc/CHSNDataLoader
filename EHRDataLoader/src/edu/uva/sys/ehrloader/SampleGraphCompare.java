...
public class SampleGraphCompare {
	...
	public static void main(String[] args) {
		...
		for (double thrr = 4.72; thrr <= 4.72; thrr += 0.001) {
			...
			try {
				ps = new PrintStream(
						"/Users/xiongha/Box Sync/CHSN_pattern mining/Jinghe/sgraph-threshold-" + thrr + ".txt");
				...
				for (int r = 0; r < 20; r++) {
					...
					for (int sampling = 100; sampling <= 200; sampling += 100) {
						...
						for (int overlap = 10; overlap <= 30; overlap += 10) {
							new GraphEva(negTruth.adaptiveThresholding(thrr / Math.sqrt(20000)),
									negWishart.adaptiveThresholdingDiff(thr, overlap)).print(
											"wishart-"+ sampling + "-" +overlap+"-neg\t" + t, ps);
							new GraphEva(posTruth.adaptiveThresholding(thrr / Math.sqrt(20000)),
									posWishart.adaptiveThresholdingDiff(thr, overlap)).print(
											"wishart-" + sampling + "-" +overlap + "-pos\t" + t, ps);
								int[][] diffWishart = posWishart.adaptiveThresholdingDiff(thr, overlap, negWishart);
								new GraphEva(diffTruth, diffWishart).print("wishart-" + sampling
										 + "-" + overlap + "-diff\t" + t, ps);
							}
						}
